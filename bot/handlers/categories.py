...
async def callback_category_selected(
    ...
    if not callback.data:
        logger.error("Callback data отсутствует")
        return

    category = callback.data.split(":")[1]
    logger.info("Пользователь %d выбрал категорию %s", callback.from_user.id, category)
    ...
    if callback.message and isinstance(callback.message, Message):
        await callback.message.edit_text(
            full_text,
            reply_markup=question_data["keyboard"]()
        )
...
async def _process_answer(
    ...
    logger.info(
        "Пользователь %d ответил на вопрос %s: %s",
...
async def _handle_processing_state(
    ...
    logger.info(
        "Пользователь %d завершил опрос категории %s",
        callback.from_user.id,
        category
    )
